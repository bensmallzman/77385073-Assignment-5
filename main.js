...
function addTax(arr)
{
    if (arr instanceof Array)
    {
        var tax = arr[1] * 0.0925;
        // ceil(tax * 100) in order to get cents
        tax = Math.ceil(tax * 100)/100;
        return arr.splice(2, 0, tax);
    }
}
...
{
    // remove whitespace and return array, then convert back to string with no separators
    str = str.split(" ").join("");
